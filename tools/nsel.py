...
def getans_mwd(pas, scl, mapd, ptag):
	tmp = scl.strip()
	tmp = tmp.decode("utf-8")
...
def getans(pas, scl, mapd, ptag):
	w, s = getans_mwd(pas, scl, mapd, ptag)
	return w
			...
			for line in frdi:
				...
				if tmp:
					...
					if tmp.startswith("<qid_"):
						rs.append(" ".join(["<qid_"+str(curid)+">", "|||", getans(cache, frdt.readline(), mapd, postagger)]))
						cache = []
						curid += 1
					else:
						tmp = tmp[tmp.find("|||")+4:]
