local FHiQATagger, parent = torch.class('nn.FHiQATagger', 'nn.SequenceContainer')
...
function FHiQATagger:updateOutput(input)
	...
	local seql = #hinput
	local _output = self:net(1):updateOutput({feat, hinput[1]})
	...
	for _, sent in ipairs(hinput) do
		if _ > 1 then
			self.cells[_]:copy(self:net(_):updateOutput({feat, sent}))
		end
	end
	...
	local stdSize = torch.LongStorage({self._totalWords, self._isize + self._csize + self._psize + self._fsize})
...
function FHiQATagger:backward(input, gradOutput, scale)
	if self.flatten then
		self.grad_output = gradOutput
	else
		local usize = gradOutput[1]:size()
	...
	local hinput, feat = unpack(input)
	local gradCache = self.CM:backward(self.cache, self.grad_output)
	self.gradPEnc = gradCache:narrow(2, self._psind, self._psize):sum(1):squeeze(1)
	self.gradFeat, self.gradCell = unpack(self.PEnc:backward({feat, self.cells}, self.gradPEnc))
	local curid = 1
	local _gP = gradCache:narrow(2, self._csind, self._csize)
	for _, nc in ipairs(self._nWords) do
		self.gradCell[_]:add(_gP:narrow(1, curid, nc):sum(1))
	...
	for _, v in ipairs(hinput) do
		local nc = self._nWords[_]
		local _curGradF, _curGrad = unpack(self:net(_):backward({feat, v}, self.gradCell[_], scale))
		_curGrad:add(_gP:narrow(1, curid, nc))
		table.insert(_gradInput, _curGrad)
		self.gradFeat:add(_curGradF)
		curid = curid + nc
...
function FHiQATagger:clearState()
	self.cells:set()
	self.cache:set()
	self._output:set()
