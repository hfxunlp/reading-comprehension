local HiQATagger, parent = torch.class('nn.HiQATagger', 'nn.SequenceContainer')

-- sentEnc and PEnc should not enable fullout
function HiQATagger:__init(SentEnc, PEnc, Classifier, flatten)
...
function HiQATagger:updateOutput(input)
	...
	local seql = #hinput
	local _output = self:net(1):updateOutput(hinput[1])
...
function HiQATagger:updateGradInput(input, gradOutput)
	if self.flatten then
...
function HiQATagger:accGradParameters(input, gradOutput, scale)
...
function HiQATagger:backward(input, gradOutput, scale)
	if self.flatten then
		self.grad_output = gradOutput
	else
		local usize = gradOutput[1]:size()
...
function HiQATagger:clearState()
	self.cells:set()
	self.cache:set()
	self._output:set()
