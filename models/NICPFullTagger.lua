local NICPFullTagger, parent = torch.class('nn.NICPFullTagger', 'nn.SequenceContainer')

-- SentEnc should enable fullout
-- PEnc should enable fullout
function NICPFullTagger:__init(SentEnc, PEnc, Classifier, flatten, ploc)
...
function NICPFullTagger:updateOutput(input)
	...
	if not self._isize then
		...
		self._sind = self.ploc + 1--可用cache起始地址,句词特征起始索引
		self._clsind = self._sind + self._csize--句特征起始索引
		...
		self._cachedim = self.ploc + self._csize * 2 + self._psize * 2 + self._fsize--cache尺寸
	end
	...
	for _, nc in ipairs(self._nWords) do
		...
		curT:narrow(2, self._sind, self._csize):copy(self:net(_).output:narrow(3, 1, self._csize))--放置句词特征
...
function NICPFullTagger:updateGradInput(input, gradOutput)
	if not self.doupgi then
		...
		_gP:zero()--清空
		self.gradSEnc:narrow(3, 1, self._csize):copy(gradCache:narrow(2, self._sind, self._csize))--传递分类器到句词特征的误差
		local _gradInput = {}
		curid = 1
		local sid = 0
		for _, v in ipairs(hinput) do
...
function NICPFullTagger:accGradParameters(input, gradOutput, scale)
	if not self.doupgi then
...
function NICPFullTagger:backward(input, gradOutput, scale)
	if self.flatten then
		self.grad_output = gradOutput
	else
		local usize = gradOutput[1]:size()
...
function NICPFullTagger:evaluate()
	parent.evaluate(self)
	self.train = true
end

function NICPFullTagger:clearState()
	self.cells:set()
	self.cache:set()
	self._output:set()
